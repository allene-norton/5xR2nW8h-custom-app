...
import { type FormType, BACKGROUND_CHECK_OPTIONS, FORM_TYPE_INFO } from "../../types"
...
export function BackgroundChecksSection({ formType, selectedChecks, updateFormData }: BackgroundChecksSectionProps) {
  ...
  const missingRequired = requiredChecks.filter((check) => !selectedChecks.includes(check))
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select the background checks to perform for this {FORM_TYPE_INFO[formType].title.toLowerCase()}
        </CardDescription>
        {missingRequired.length > 0 && (
          <div className="flex items-start space-x-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <AlertTriangle className="w-4 h-4 text-yellow-600 mt-0.5" />
            <div className="text-sm">
              <p className="font-medium text-yellow-800">Missing Required Checks</p>
              <p className="text-yellow-700">The following checks are required: {missingRequired.join(", ")}</p>
            </div>
          </div>
        )}
      </CardHeader>
            ...
            return (
              ...
              >
                <Checkbox
                  id={`check-${checkName}`}
                  checked={isChecked}
                  onCheckedChange={(checked) => handleCheckChange(checkName, checked as boolean)}
                  className="mt-1"
                />
