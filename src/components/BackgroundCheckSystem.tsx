...
interface BackgroundCheckSystemProps {
  initialView?: "admin" | "client"
  className?: string
}

export function BackgroundCheckSystem({ initialView = "admin", className = "" }: BackgroundCheckSystemProps) {
  const [currentView, setCurrentView] = useState<"admin" | "client">(initialView)
  const {
    formData,
    isLoading,
    lastSaved,
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{currentView === "admin" ? "Admin Interface" : "Client Portal"}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`text-sm ${currentView === "admin" ? "font-medium text-blue-600" : "text-gray-500"}`}
                  >
                    Admin
                  </span>
                  <Switch
                    id="view-toggle"
                    checked={currentView === "client"}
                    onCheckedChange={(checked: any) => setCurrentView(checked ? "client" : "admin")}
                  />
                  <span
                    className={`text-sm ${currentView === "client" ? "font-medium text-blue-600" : "text-gray-500"}`}
                  >
                    Client
                  </span>
                </div>
